...
def GUI(self, Gtk, vboxStack21, fn):
    ...
    hbox2_lbl.set_markup("Discover other computers in your network (to access other computers)")
    ...
    hbox13_lbl.set_markup("3. Create a password for the current user to be able to access the Samba server")
    ...
    hbox14_lbl.set_markup("You can now reboot and enjoy the <b>'Shared'</b> folder if you choose '<b>easy</b>' ")
    ...
    hbox15_lbl.set_markup("If you choose '<b>usershares</b>' then we recommend you install also thunar and its plugin and \
right-click to share any folder in your home directory\nThere are other filemanagers with their plugins at the bottom")
    ...
    hbox90_lbl.set_markup("<span foreground=\"red\" size=\"large\">We found a firewall on your system</span>")
    ...
    hbox91_lbl.set_markup("With the Avahi daemon (network discovery) running on both the server and client,\nthe file manager on the client should automatically find the server- Beware of firewalls")
