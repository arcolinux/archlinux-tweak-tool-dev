...
def GUI(self, Gtk, vboxStack14, fn):
    ...
    button_install_discovery.connect(
        "clicked", self.on_install_discovery_clicked)
    button_remove_discovery = Gtk.Button(label="Uninstall network discovery")
    button_remove_discovery.connect(
        "clicked", self.on_remove_discovery_clicked)
...
Follow the instruction numbers below - <b>we recommend the easy configuration</b>")
    ...
    button_create_samba_user.connect(
        "clicked", self.on_click_create_samba_user)
...
their plugins at the bottom")
    ...
    button_delete_samba_user = Gtk.Button(
        label="Remove the selected user from Samba")
    button_delete_samba_user.connect(
        "clicked", self.on_click_delete_samba_user)
    button_delete_user = Gtk.Button(
        label="Remove the selected user completely")
...
Beware of firewalls")
    ...
    hbox93_label.set_text("Samba : " + status1 +
                          "   Nmb : " + status2 + "   Avahi : " + status3)
    hbox93.pack_start(hbox93_label, False, False, 10)
    hbox94_label.set_text("Samba : " + status1 +
                          "   Nmb : " + status2 + "   Avahi : " + status3)
