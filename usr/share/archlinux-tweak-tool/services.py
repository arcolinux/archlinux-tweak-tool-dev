...
def choose_smb_conf(self,widget):
    ...
    if choice == "ArcoLinux":
        ...
        GLib.idle_add(fn.show_in_app_notification, self, "Smb.conf usershares configuration applied")
    elif choice == "Windows":
        fn.copy_samba("windows")
        GLib.idle_add(fn.show_in_app_notification, self, "Smb.conf windows configuration applied")
    elif choice == "Original":
        fn.copy_samba("original")
        print("Smb.conf from gitlab of Samba")
        GLib.idle_add(fn.show_in_app_notification, self, "Smb.conf from gitlab of Samba")
...
def create_samba_user(self,widget):
    ...
    if username:
        ...
        try:
            ...
            GLib.idle_add(fn.show_in_app_notification, self, "Created a password for the current user")
...
def delete_samba_user(self,widget):
    ...
    if username:
        ...
        GLib.idle_add(fn.show_in_app_notification, self, "Deleting the selected user from Samba...")
    else:
...
def restart_smb(self,widget):
    ...
    if fn.check_service("smb"):
        ...
        GLib.idle_add(fn.show_in_app_notification, self, "Did you install samba - check for errors")
